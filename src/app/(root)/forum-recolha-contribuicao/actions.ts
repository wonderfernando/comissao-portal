...
export async function getAllForumContribuicoesAction(): Promise<{ success: boolean; dados: Forum[] }> {
    await new Promise(resolve => setTimeout(resolve, 5000));
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/forun/contribuicao`, {
        method: "GET",
        headers: {
            "Content-Type": "application/json",
            ...(await getAuth())
