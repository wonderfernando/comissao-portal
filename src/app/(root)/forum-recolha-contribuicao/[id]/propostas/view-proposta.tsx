...
export default function ForumContribuicoes({ dados }: { dados: Forum }) {
  ...
  return (
    <div className="mx-auto py-10 px-4">
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl font-bold">Propostas aprovadas</CardTitle>
        ...
        <CardContent>
          <h3 className="text-lg font-semibold mb-4">Projectos</h3>
          {dados.forun_contribuicao.length === 0 ? (
            <p className="text-muted-foreground">Nenhuma contribuição registrada.</p>
          ) : (
            <div className="space-y-4 mb-6">
              {dados.forun_contribuicao.map((item) => (
                <div key={item.id} className="border rounded-lg p-4">
                  <div className="flex items-center justify-between">
                    <span className="text-xs"> Prioridade: {statePriority(item.prioridade)}</span>
                    <span className=" text-muted-foreground">
                      Criado em {new Date(item.created_at).toLocaleString("pt-BR", { dateStyle: "short", timeStyle: "short" })}
                    </span>
                  </div>
                  <Separator className="my-2" />
                  <p className="font-semibold text-sm text-zinc-400">Descrição da Proposta:</p>
                  <p className="text-sm mb-2">{item?.proposta?.titulo}</p>
                  <Separator className="my-2" />
