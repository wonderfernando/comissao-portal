...
export function RelatorioFinanceiroView({ dados }: { dados: Forum | null }) {
    ...
    // Calculate total estimated cost from all proposals
    console.log(dados)
    const propostas = dados.forun_contribuicao || []
    const totalCusto = propostas.reduce((acc, item: any) => {
        const custo = item?.proposta?.custo_estimado || "0"
    ...
    const propostasPorCategoria = propostas.reduce((acc: any, item: any) => {
        const categoriaNome = item?.proposta?.categoria?.nome || "Sem categoria"
                                    ...
                                    {propostas.map((item: any, index: number) => (
                                        <TableRow key={item.id}>
                                            ...
                                            <TableCell className="font-medium">
                                                {item?.proposta?.titulo || "Sem título"}
                                            </TableCell>
                                            <TableCell>
                                                <Badge variant="secondary">
                                                    {item?.proposta?.categoria?.nome || "Sem categoria"}
                                                </Badge>
                                            </TableCell>
                                            <TableCell>{getPriorityBadge(item.prioridade)}</TableCell>
                                            <TableCell className="text-right font-semibold">
                                                {formatCurrency(Number(item?.proposta?.custo_estimado || 0))}
                                            </TableCell>
                ...
                <CardContent className="pt-6">
                    <div className="flex items-start gap-4">
                        <div className="flex-1">
                            ...
                            <p className="text-sm text-muted-foreground">
                                Este relatório apresenta o valor total estimado para os projectos aprovados no fórum de recolha de contribuições
