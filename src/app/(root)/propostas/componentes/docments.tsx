...
export default function FileUpload() {
  ...
  const { append,remove, fields } = useFieldArray<z.infer<typeof PropostaCreateSchema>>({
    name: "uploads",
  })

  const {watch, register} = useFormContext<z.infer<typeof PropostaCreateSchema>>()
                ...
                {fields.map((file, index) => (
                  <TableRow key={file.id}>
                    ...
                    <TableCell className="py-2 text-muted-foreground">
                      <Input {...register(`uploads.${index}.descricao`)} />
                      {errors?.uploads?.[index]?.descricao?.message}
                    </TableCell>
                    <TableCell className="py-2 text-muted-foreground">
