...
import { Proposta, PropostaCtgom } from "../propostas/interface"
import { usePathname } from "next/navigation"

export const columns: ColumnDef<PropostaCtgom>[] = [
  {
    accessorKey: "titulo",
    header: "Título",
  },
  ...
  },
  {
   ...
   {
    accessorKey: "nivel_percetagem",
    header: "Nivel de Execução",
    cell: ({ row }) => (
      <Badge variant="outline" className="capitalize">
        {row.original?.nivel_percetagem} %
      </Badge>
    ),
  },
  ...
  },
  {
   ...
   {
    accessorKey: "ctgom.nome",
    header: "CTGOM",
    cell: ({ row }) => {
      return row.original.ctgom?.nome || "N/A"
    }
  },
  {
    accessorKey: "anexos",
...
export default function TablePropostas( { className = "" , propostas}: { className?: string , propostas: PropostaCtgom[] }) {
  const id = useId()
                ...
                >
                  {row.getVisibleCells().map((cell) => {
                    if (cell.column.id === "titulo") {
                      return (
                         <TableCellSuspense  href={`${url}/${propostas?.[index]?.id}`} key={cell.id}>
                        {flexRender( 
